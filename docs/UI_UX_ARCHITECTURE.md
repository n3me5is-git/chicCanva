...
The semantic reference annotation editor is a focused overlay rather than a sidebar workflow. On desktop it keeps a small outer margin and rounded corners; mobile uses the full viewport. A compact top toolbar leaves the image as the dominant surface. Marker comments appear in a bottom overlay panel that never resizes the canvas, opens automatically for a new marker, and exposes an annotation list only when nothing is selected. Destructive delete-all remains behind right-click or long press on the selected-marker delete command. The page Preview overlay follows the same desktop margin and mobile full-screen rule.
