...
MEGAJS `MutableFile.delete(false)` moves a node to the Trash while `delete(true)` removes it permanently. Manual explorer/copy/cleanup actions expose that distinction; automatic revision retention, expired leases, aborted uploads, and content-addressed garbage collection use permanent deletion to avoid filling Trash with implementation artifacts.

The explorer temporarily hosts the shared tooltip in the modal top layer and restores it on close or Escape. Selecting a folder exposes an Open action in its details; double-click uses the same navigation helper. Dot-prefixed nodes, including `.backups` and `.synch`, are always included without filtering. Explorer-to-copy navigation passes stable folder names, then reloads manifests and preselects the scoped Backup instance or exact Synch revision without downloading workspace payloads. Image previews show encoded file size and decoded pixel dimensions on a checkerboard; PDF frames expand into the remaining preview height. A generation counter rejects stale preview downloads after another selection or close. Memory settings use a separate live connection status, preserving the backup result log. The bilingual account explanation links to MEGA and the guide recommends a dedicated account; saved session data are encrypted locally and cloud files are transferred directly to MEGA.


Cloud recovery and Cloud Copies use a shared MEGAJS stream reader. It aggregates byte-level `progress` events across the workspace file and recursively referenced content-addressed nodes, while reserving the final range for reconstruction and adoption.

## Cloud content-addressed storage

Cloud v2 recursively packs large strings and JSON subtrees into immutable SHA-256-addressed objects. Parent nodes refer to child hashes, so a change uploads the changed branch and its small ancestor chain while reusing unaffected assets, pages, and projects. Backup indices and per-revision Synch manifests are the reachability roots. Recovery recursively resolves those nodes into the ordinary workspace schema before opening or exporting. Portable compressed project export uses the browser CompressionStream/DecompressionStream GZIP implementation and adds no runtime dependency.
