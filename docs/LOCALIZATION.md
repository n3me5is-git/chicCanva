...
- `development/guide_content.py` is the shared bilingual editorial source for chapters, feature topics, guided projects, glossary, and FAQ.
- `development/guide_manual.py` renders that source and creates localized embedded SVG illustrations, faithful non-interactive UI mockups, icon atlases, and arrow workflows.
- `development/build-guide.py` and `development/build-guide-en.py` generate the Italian and English fragments.
- `development/guide-manual.css` supplies the shared responsive handbook layout.

Both languages are rendered from the same semantic structure, so topic IDs, internal links, illustrations, option tables, examples, glossary entries, and FAQs remain in parity. The build embeds both and mounts the active fragment. `build-docs.py` publishes the exact same fragments as `docs/user-guide.html` and `docs/user-guide-en.html`, with all illustrations embedded as data URLs. Update the shared content whenever a feature changes; never hand-edit generated guide HTML.
...
Semantic reference annotations follow that exception narrowly: the structural instruction that explains Image 1, Image 2, normalized coordinates, and preservation rules is authored in Italian and English and selected from the active application locale. Marker type keys and coordinate field names remain stable technical values. The user's marker comments are appended verbatim inside the stable `user_comment: {…}` delimiter and are never translated. The raw-prompt view must show the exact localized structural block that will be submitted.
