...
Classic grid snap and magnetic guide snap are separate systems. The guide engine reads the bounding rectangle of the moving Fabric target, so a single object, a native group, and an `ActiveSelection` all follow the same path. It compares page centers, all four page edges, and the left/center/right or top/center/bottom anchors of non-selected visible objects within an eight-screen-pixel magnetic threshold. Page-center and page-edge guides can be toggled independently. When an object lies between neighbors, it also compares the two gaps and snaps to equal spacing. The same page-edge candidates are available to resize and crop. Guide lines are temporary, non-evented Fabric objects marked `excludeProject` and `excludeFromExport`; they are removed on pointer-up, modification, selection clearing, page serialization, and export.

The object context menu opens a lateral **Fit selection** submenu. **Fit with margin** reads the persisted millimetre value shown immediately after the rotation-step setting; **Fit without margin** uses the full page bounds. Both use proportional contain fitting, transform objects around the actual collective bounding-box center regardless of Fabric origin, and restore the resulting selection. A matching-aspect image therefore reaches all four page edges in zero-margin mode; differing aspects leave space on one axis rather than stretch or crop.
...
The **Change quality** command replaces one selected object after explicit confirmation. Resolution factors are ¼×, ⅓×, ½×, ¾×, 1×, 1¼×, 1½×, 2×, 2½×, 3×, 3½×, and 4×. Downscaling uses progressive high-quality canvas passes, never reducing by more than approximately one half per pass before the exact final size; this avoids the visibly poor result of one large browser reduction and better preserves text edges. Upscaling uses one high-quality pass. Crop coordinates and source dimensions are multiplied by the factor while Fabric scale is divided by it, preserving page position and physical size. Temporary bitmap/canvas resources are closed or reduced immediately. Upscaling improves pixel count for compatibility but does not reconstruct detail.

PNG quality is a separate post-resampling stage. At 100%, the resampled colors are encoded unchanged. Lower values quantize RGB precision before PNG encoding, reducing compressed asset, autosave, and JSON size while preserving alpha transparency and PNG output. The 1× factor permits storage optimization without changing pixel dimensions. Both factor and PNG quality are stored in derived-asset metadata.
...
Generated visible text has no automatic language instruction. When wording is required, the user states the exact text and language in the prompt and can save reusable instructions in Prompt Library. Reference images may come from file, URL, clipboard, or the selected canvas image. A 0.25×, 0.5×, 0.75×, 1×, 2×, or 3× control creates a temporary high-quality PNG copy for the request; 1× is the default and the original asset remains unchanged. “Prepare coloring image” fills the widget with GPT Image 2.5 Flare, low quality, the coloring-page preset, the selected object as reference, and a tailored prompt. It never starts generation automatically.

With GPT Image 2.5 Flare or Sunburst, the separate reference-annotation editor can place rectangles, ovals, and fixed-size numbered spots over the reference. Its compact top toolbar controls tools, zoom, fit, selected-marker deletion, and a white/black transparency-preview background. The stage itself stays white by default while the area outside the image is gray; changing the preview background never changes source pixels or the submitted raster. The background control is enabled only when the decoded preview contains transparent pixels; opaque references keep it disabled and white. Comments use an overlay bottom panel: drawing a marker returns to Select and opens its comment automatically; with no marker selected the panel lists all annotations. Right-click or long-press on Delete exposes the separate delete-all command. Rectangle and oval minimum sizes are enforced in interface pixels, and point markers expose a normalized center only. Save stores normalized vector data with the project. Close without saving discards the current editing session. An incompatible model disables editing without deleting saved markers; a replaced or removed reference clears them.

For an annotated generation, the app sends the clean scaled reference and a temporary rasterized annotated copy in one documented Puter `input_images` request. The localized technical prompt block describes the two image roles, normalized geometry, and marker comments. Each unchanged user comment is delimited as `user_comment: {…}` instead of a quoted JSON string; the raw-prompt command displays that exact append. The price estimate counts two same-size image inputs. No extra request occurs, and the temporary annotated bitmap is not persisted.

The estimate above the generation button uses published prices and observed Puter usage rows. It reports the dashboard's Credits scale, an indicative dollar value, approximate prompt tokens, output dimensions, and reference dimensions after scaling. GPT Image 2.5 output tokens follow OpenAI's public non-linear calculator formula. Prompt tokens remain a character-count heuristic; reference-image input uses a clearly labelled Vision-derived patch proxy because OpenAI explicitly excludes GPT Image generation/editing from the Vision calculator. For OpenAI, quality remains independent from size. The preset selector chooses an approximate minimum, 0.8K, 1K, 1.5K, or 2K short edge; the separate exact mode accepts explicit width and height and disables aspect and preset controls. `aiOpenAiDimensions()` and `aiOpenAiCustomDimensions()` enforce multiples of 16, the 1:3–3:1 aspect range, 3840 px edge maximum, and 655,360–8,294,400 pixel range. Invalid changes restore the previous valid combination and display a temporary warning. xAI exposes its distinct 1K/2K tier. Puter has no documented `txt2img()` preflight quote, so `~` is deliberate and final account usage remains authoritative.
...
The user-facing result page is capped at 12 unique rendered cards and every thumbnail uses native lazy loading. Openclipart currently returns up to 32 records in one remote page; chicCanva parses that page once, slices it into 12-card local pages, and keeps parsed results in a session cache until a new search begins. Previous/next therefore reuses both records and browser-cached thumbnails whenever possible. Crossing a remote boundary fetches only the next required Openclipart page.

The explorer scrapes public Openclipart search pages because there is no embedded full catalog or required API key. Users are encouraged to search in English. Automatic translation is enabled by default and uses the compact built-in dictionary first, then the persistent successful-translation cache. Fully known and cached phrases never leave the browser. Unknown text can use Gemma 4 31B through Puter when the user is already signed in; otherwise known words are translated locally and unknown words are preserved. A parenthesized suffix is a disambiguation hint rather than search output: `sirena (creatura mitologica)` can select `mermaid` without appending the hint. The cache stores only the normalized searchable phrase, excluding the hint, so the successful result is reusable for the same word later. The two search widgets expose a shared cache-clear command that does not touch projects or other settings. The compact prompt is `"<query>" -> translate IT to EN. Text in (...) is context only: use it to choose meaning, do not output it. Output translated search text only. If input is already EN, output it unchanged. Choose one best synonym. Context: emoji/clipart search for drawing, creative and educational projects.`. The preferred call uses `provider: "openrouter"`, `normalize: true`, and `reasoning: { effort: "low" }`. If the OpenRouter request fails or produces no valid final text, chicCanva retries once with the same Gemma model and `normalize: true` through Puter's default routing, omitting both provider and reasoning options. Puter's generic documentation does not list Gemma among the families covered by `reasoning_effort`, but a live browser probe on 2026-09-10 confirmed that the OpenRouter route accepts the nested reasoning option. The response parser still removes `<thought>`, `<thinking>`, `<reasoning>` and `<analysis>` blocks.

## PWA installation flow

On a public HTTPS domain the toolbar Install command always opens chicCanva's installation dialog. When Chromium supplies a `beforeinstallprompt` event, the dialog exposes **Install now**, which triggers the browser's native installation prompt. If that event is unavailable, the same dialog shows platform-specific manual steps; iOS keeps the Safari Share → Add to Home Screen route. File, localhost, IP and private-host launches remain outside PWA mode.
...
During pinch, the viewport transforms the common canvas shell inside `requestAnimationFrame` without rebuilding the Fabric backing raster. A shared multi-touch gate cancels any transform, Colorizer stroke or delayed fill, shape draft, export-region draft, or delayed eyedropper sample begun by the first finger, then blocks those tools until all touch pointers are released. On gesture completion it removes the temporary transform, recalculates Fabric offsets and performs one retina-aware refresh. Manual zoom preserves the page-space center, while non-fit mode adds one full viewport of navigation padding on each side so the hand tool can move every canvas edge into reach horizontally and vertically.
...
`copySelectionMultipurpose()` is shared by the toolbar, object context menu, and Ctrl/Cmd+C. It records serialized objects and referenced assets in the internal clipboard, then attempts to expose a PNG of that exact selection to external applications. For a single unrotated image, the external image is extracted from the original asset at native crop resolution; the structured copy still carries the complete asset and crop metadata. The image inspector places **Copy image** beside resolution replacement: it copies the original directly when no crop exists and exposes an **Original / Crop** menu only when both choices are meaningful. An external clipboard-permission failure never invalidates the structured copy. Standard paste prioritizes the internal payload and reads an OS clipboard image only if no compatible chicCanva copy exists. Dedicated image-paste buttons remain explicit raster imports.
...
## Native sharing, PWA share target, and P2P transfer

Download and operating-system sharing use the same artifact builders. `buildUnifiedExportArtifact()`, `projectExportArtifact()`, and `promptLibraryExportArtifact()` return a name, MIME type, and `Blob`; the download and Web Share entry points then choose how to deliver that exact artifact. This prevents the shared file from differing from the downloaded file. Share controls start hidden and appear only when the secure-context browser reports both `navigator.share` and a successful `navigator.canShare({files})` probe for the exact prospective MIME type. This is capability detection rather than desktop/mobile or operating-system detection: PDF or image sharing can be available while JSON or ZIP sharing remains unavailable. A final probe against the completed file still guards delivery, and a localized recovery dialog offers an ordinary download if capability changes or sharing fails.

The installed PWA declares a multipart `share_target`. Its service worker accepts only the share-target POST route, stores the received files in the temporary `chicCanva-share-inbox` IndexedDB database, and redirects to the application with an opaque inbox ID. Startup processes that ID after the saved-workspace recovery dialog. Project JSON is validated and opened as another project; Prompt Library JSON offers append or complete replacement; PDF reuses the existing page/range, destination, orientation, and quality dialog; and an image can enter the current page, a following page, or a new project. Cancel and successful completion delete the inbox record. The service worker expires abandoned records after 24 hours and rejects a share larger than 256 MB.

P2P Project Sharing is an explicit export-section workflow. The sender selects open projects and optionally the Prompt Library. It creates a checksum-protected, human-readable one-use code and an equivalent QR payload. Both peers derive the Trystero room, password, and identical five-relay subset from that code. The receiver first gets a manifest, selects wanted items, and only then requests payloads. Each payload is accepted only if its byte length and SHA-256 digest match the manifest. Imported project names receive the localized `(Importato)` / `(Imported)` suffix and a number when needed. Prompt Library data uses the same append/replace operation as manual file import.

The modal reports relay connection separately from waiting for the other device. If none of the configured sockets opens within the bounded signaling deadline, it displays a localized error and a link to relay settings. A user-provided `wss://` relay is stored only in that browser and is added to the internal relay subset, so differently configured devices still retain common fallback relays. The session closes after five minutes without activity, on dialog close, or after a completed receive. Camera QR scanning starts only after the receiver presses the scan button and stops tracks on close, success, cancellation, or error. A QR URL can prefill the receiver code, but never starts a connection or import automatically.

### Startup canvas stabilization

The same light-preview and final-raster boundary used by sidebar resizing applies while the saved-project startup dialog closes and the recovered workspace loads. Resize notifications during that interval update only CSS presentation, are coalesced, and end in one Fabric raster rebuild after layout stability. The current Fit or numeric zoom mode remains unchanged.

## MEGA Cloud Storage

`development/cloud-storage.js` uses the vendored MEGAJS client only after an explicit sign-in. Its capability gate checks that same bundled client together with the secure-context flag and Web Crypto. It accepts a real HTTPS origin or the loopback development hosts `localhost`, `127.0.0.1`, `::1`, and `[::1]`; ordinary HTTP domains and LAN addresses remain unavailable. The sidebar reports account quota and exposes a custom chicCanva explorer. Account root is readable; full create, rename, clone, move, and delete operations are limited to `chicCanva Cloud` and its descendants. File selection downloads into a bounded `File` and reuses the established image, PDF, project, and Prompt Library import flows. Exports first choose device or Cloud, then reuse the same artifact builders; rendering does not begin before that choice.

The optional browser session stores MEGA's resumable session JSON encrypted with AES-GCM. A non-extractable `CryptoKey`, IV, and ciphertext are stored in a separate IndexedDB database. Password and 2FA code are cleared after login and never serialized. Logout closes the client and removes both key and ciphertext.

Automatic Backup runs only after a successful local autosave, only for a changed payload, and at most once every ten minutes. Enabling Backup or Sync requests that first local autosave. When no previous Cloud timestamp exists, the initial upload is scheduled 30 seconds after the first committed change. Later autosaves replace the single pending serialized payload without allocating a queue and without postponing that deadline. When the ten-minute rate limit ends later, that time remains the effective deadline. The Memory dialog displays the queued deadline and live upload percentage. Automatic preparation yields to the browser's scheduler or an idle callback before hashing and between projects, while network uploads remain asynchronous. It writes project files under `chicCanva Cloud/.backups/<instance>/projects`, uploads a new index only after the files succeed, and retains existing cloud data on failure. Automatic Sync applies the same modification-driven, ten-minute queue and hashes the complete workspace to avoid duplicate revisions. It writes one JSON per project plus a full workspace recovery record under a unique immutable `.synch/<session>/revision-*` directory, then replaces `current.json` last. A 90-second writer lease is checked after refresh and released in `finally`; its expiry prevents an abandoned client from blocking the session. The remote manifest is compared before the lease and immediately before publication. Polling ignores revisions carrying the current browser's writer ID. A newer revision from another writer suspends the stale client before it can upload and shows the decision once. Choosing local-only keeps Sync suspended without repeated prompts until the user explicitly opens the crossed-cloud badge or reloads the application. Startup discovers the newest `.synch` session after MEGA reconnection; Cloud recovery keeps the startup dialog open and shows staged loading progress.

The final memory-optimized `persistProject()` override is the authoritative autosave writer. After its IndexedDB generation and local pointer succeed, it must forward that exact serialized payload and timestamp to `chicCloudAutosaveCommitted()`. Tests must exercise this final runtime path from a Fabric `object:modified` event; calling the Cloud scheduler directly does not prove the integration.

The Memory dialog exposes separate Backup and Synch status rows. Each has a manual **Run now** action and a filtered log of the last 100 attempts. Manual runs bypass unchanged-payload deduplication and the automatic ten-minute queue, but Synch still enforces suspension, remote-revision and lease checks. Log records contain timestamp, source, outcome and a bounded error/result description; they remain in browser localStorage and never contain project payloads or MEGA credentials. Every attempted Backup or Synch refreshes MEGA account quota so the Cloud sidebar reflects updated usage as soon as the service reports it. Explorer filesystem mutations and its Refresh action do the same; the quota panel also provides an explicit Refresh storage link.

At startup a restored MEGA session can supply a cloud workspace after local recovery is inspected. After a manual login, the same discovery scans `.synch` and offers the newest valid session even when the new browser has no local session ID. The user can open the local copy, replace it with the cloud copy, or open both sets of projects together. Reload from Cloud remains able to read while Synch is suspended and adopts the remote session before reloading; Continue locally keeps the warning available so the user can reattach later. A deliberately new workspace receives a new session ID. No network login or cloud upload starts when session saving and cloud options are disabled.


### Cloud explorer preview and connection feedback

The explorer temporarily hosts the shared tooltip in the modal top layer and restores it on close or Escape. Selecting a folder exposes an Open action in its details; double-click uses the same navigation helper. Dot-prefixed nodes, including `.backups` and `.synch`, are always included without filtering. Browse copies opens an already filtered catalogue and selects the relevant Backup instance or exact Synch revision. Image previews show encoded file size and decoded pixel dimensions on a checkerboard; PDF frames expand into the remaining preview height. A generation counter rejects stale preview downloads after another selection or close. Memory settings use a separate live connection status, preserving the backup result log. The bilingual account explanation links to MEGA and the guide recommends a dedicated account; saved session data are encrypted locally and cloud files are transferred directly to MEGA.

Backup scheduling keeps one cloud operation in flight and coalesces autosaves received during an upload into the latest pending snapshot. After completion the next timer uses the successful backup timestamp, preserving the ten-minute interval even on slow connections. Regression tests use an isolated MEGA node fixture to check separate project/history files, unchanged-payload deduplication, failed index publication, upload-before-cleanup ordering, retention of closed projects and slow-upload scheduling. No test writes to a real account.

### Cloud storage format v2

Backup and Synch recursively externalize strings, arrays, pages, projects, and object trees of at least 64 KiB into immutable SHA-256-addressed nodes. A small edit creates only the changed leaf and ancestor nodes; unchanged assets and document branches are reused. Synch keeps a manifest inside every revision and publishes `current.json` last. Manifests include project names, page names/counts, and logical sizes, allowing startup, polling, conflict detection, and the Cloud Copies browser to remain metadata-only. Full nodes are downloaded only after an explicit Open, Export, or Reload from Cloud action. Those downloads use MEGAJS stream progress events and aggregate transferred bytes across the workspace and referenced objects, reserving the final progress range for reconstruction and adoption. Backup indices retain the latest 100 revision descriptors. The Cloud Copies dialog can delete one Backup revision or a selected Synch session; removal of a linked Synch session disables synchronization on every client at its next successful metadata poll. The dialog opens selected projects through the normal new-project flow and exports a monolithic `.json` or `.json.gz` through the existing device/Cloud destination flow. The same dialog opens from Memory, Cloud Storage, and qualifying folders in the Cloud explorer; explorer entry preserves the selected folder scope. Every manual deletion offers MEGA Trash or permanent removal and explains that Trash recovery is available only in official MEGA clients. Automated retention, stale locks, aborted revisions, and unreachable-object garbage collection remain permanent technical cleanup. Reachability cleanup removes revision files beyond retention and deletes content-addressed objects only when no retained Backup or Synch revision references them. Cloud v1 and the provisional v2 format are intentionally not read.

Project export optionally writes the complete project as lossless GZIP (.json.gz), enabled by default. Import detects the GZIP magic bytes rather than trusting the filename and continues to accept plain .json.
