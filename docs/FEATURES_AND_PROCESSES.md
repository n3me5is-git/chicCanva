...
Automatic Backup runs only after a successful local autosave, only for a changed payload, and at most once every ten minutes. It writes project files under `chicCanva Cloud/.backups/<instance>/projects`, uploads a new index only after the files succeed, and retains existing cloud data on failure. Automatic Sync writes one JSON per project plus a full workspace recovery record under `.synch/<session>/revision-*`, then replaces `current.json` last. A short writer lease and revision manifest implement best-effort single-writer coordination. A newer remote writer suspends local sync, requires a reload/local-only decision, and leaves an out-of-sync badge unless the user disabled it.

At startup a restored MEGA session can supply a cloud workspace after local recovery is inspected. The user can open the local copy, replace it with the cloud copy, or open both sets of projects together. No network login or cloud upload starts when session saving and cloud options are disabled.
