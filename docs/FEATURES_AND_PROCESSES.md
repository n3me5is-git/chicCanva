...
P2P Project Sharing is an explicit export-section workflow. The sender selects open projects and optionally the Prompt Library. It creates a checksum-protected, human-readable one-use code and an equivalent QR payload. Both peers derive the Trystero room and password from that code. The receiver first gets a manifest, selects wanted items, and only then requests payloads. Each payload is accepted only if its byte length and SHA-256 digest match the manifest. Imported project names receive the localized `(Importato)` / `(Imported)` suffix and a number when needed. Prompt Library data uses the same append/replace operation as manual file import.

The session closes after five minutes without activity, on dialog close, or after a completed receive. Camera QR scanning starts only after the receiver presses the scan button and stops tracks on close, success, cancellation, or error. A QR URL can prefill the receiver code, but never starts a connection or import automatically.
