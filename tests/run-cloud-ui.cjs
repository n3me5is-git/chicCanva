...
(async()=>{
 ...
 try{
  ...
  for(const lang of ['it','en']){
   ...
   await view.locator('#cloudStorageCard').screenshot({path:path.resolve('development/guide-assets/'+lang+'-cloud-panel.png')});
   await view.evaluate(()=>document.getElementById('settingsDialog').showModal());
   await view.locator('#settingsDialog').screenshot({path:path.resolve('development/guide-assets/'+lang+'-memory-dialog.png')});
   await context.close();
  }
