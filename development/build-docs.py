...
@media(max-width:850px){.guide-shell{display:block}.guide-toc{position:sticky;top:8px;z-index:5;max-height:min(55dvh,420px);margin-bottom:18px;box-shadow:0 6px 18px #193c3424}.guide-toc:not([open]){padding-bottom:14px}.guide-body>section{scroll-margin-top:92px}}
...
""" + (ROOT / "development" / "guide-manual.css").read_text(encoding="utf-8")
    script = """<script>document.addEventListener('click',function(event){var link=event.target.closest('.guide-toc a');if(link&&matchMedia('(max-width:900px)').matches){var toc=link.closest('.guide-toc');if(toc)toc.open=false;}});</script>"""
    html = f"""<!doctype html>
...
<main>{fragment}</main><a class="back" href="#top" aria-label="{escape(back_label)}">{escape(back)}</a>{script}</body></html>"""
