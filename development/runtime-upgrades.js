...
function bindPinchZoom(){
 ...
 const apply=()=>{raf=0;if(!pinch||!latest)return;const view=pinchViewport(pinch,latest[0],latest[1]),rect=wrap.getBoundingClientRect();zoomMode=String(view.zoom);fitStageZoom();const shell=shellOffset();wrap.scrollLeft=shell.x+pinch.localX*view.zoom-(view.mid.x-rect.left);wrap.scrollTop=shell.y+pinch.localY*view.zoom-(view.mid.y-rect.top);latest=null};
