...
function collectAssetIdsFromObjects(objects,ids){for(const object of objects||[]){if(object.assetId)ids.add(object.assetId);if(object.children)collectAssetIdsFromObjects(object.children,ids)}}
function collectPayloadAssetIds(payload,includeHistory=true){const ids=new Set();for(const pages of Object.values(payload?.pages||{}))for(const page of pages)collectAssetIdsFromObjects(page.objects,ids);if(includeHistory)for(const entry of payload?.history?.entries||[])for(const pages of Object.values(entry.pages||{}))for(const page of pages)collectAssetIdsFromObjects(page.objects,ids);return ids}
...
function pruneUnreachableAssets(){
 saveActivePage();let removed=0;const current=activeProject();if(current){const keep=new Set();for(const pages of Object.values(state.pages))for(const page of pages)collectAssetIdsFromObjects(page.objects,keep);for(const entry of historyEntries)for(const pages of Object.values(entry.pages||{}))for(const page of pages)collectAssetIdsFromObjects(page.objects,keep);for(const id of Object.keys(state.assets))if(!keep.has(id)){delete state.assets[id];removed++}current.payload=projectSnapshotForWorkspace(true)}
