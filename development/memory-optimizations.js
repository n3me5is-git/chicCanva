...
function pruneUnreachableAssets(){
 saveActivePage();let removed=0;const current=activeProject();if(current){const keep=new Set();for(const pages of Object.values(state.pages))for(const page of pages)collectAssetIdsFromObjects(page.objects,keep);for(const entry of historyEntries)for(const pages of Object.values(entry.pages||{}))for(const page of pages)collectAssetIdsFromObjects(page.objects,keep);expandDerivedAssetDependencies(keep,state.assets);for(const id of Object.keys(state.assets))if(!keep.has(id)){delete state.assets[id];removed++}current.payload=projectSnapshotForWorkspace(true)}
