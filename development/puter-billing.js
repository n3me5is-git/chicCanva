...
const PUTER_IMAGE_PRICING_FALLBACK=Object.freeze({
 ...
 models:{
  ...
  'openai/gpt-image-2':{kind:'openai-token-image',textInputPer1M:2.5,imageInputPer1M:4,imageOutputPer1M:15,qualityMultiplier:{low:1,medium:9.01}},
...
updateAiEstimate=function(){
 ...
 const listEstimate='~ '+aiCreditText(q.usd)+' · '+aiUsdText(q.usd),actual=$('aiLastGeneration'),last=puterActualCostCache()[aiEstimateSignature(q)];$('aiCostEstimateValue').textContent=last?'Ultima Gen: '+Math.round(last.credits).toLocaleString(typeof appLocale==='function'?appLocale():'it-IT')+' crediti · '+aiUsdText(last.usd):'Stima Listino: '+listEstimate;$('aiCostEstimateDetail').innerHTML=parts.join(' · ')+'<br>'+details+' · confidenza '+q.confidence;if(actual){actual.classList.toggle('hidden',!last);actual.textContent=last?'Stima Listino: '+listEstimate:''}
};
