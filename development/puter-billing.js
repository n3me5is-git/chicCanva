...
function renderPuterBilling(snapshot){
 ...
 $('puterUsageUsed').textContent='Usato '+puterBalanceLabel(consumed.value,unit);$('puterUsageRemaining').textContent='Disponibile '+Math.round(percent)+'% · '+puterBalanceLabel(remaining,unit);
 $('puterAccountType').textContent=user?.subscribed===true?'Subscription':'Free / senza abbonamento';$('puterMonthlyAllowance').innerHTML='<strong>'+puterBalanceLabel(monthlyAllowance,unit)+'</strong><span>Usati: '+puterCreditsLabel(allowanceUsed,unit)+' · Residui: '+puterCreditsLabel(monthlyRemaining,unit)+'</span>';$('puterTopupCredits').innerHTML='<strong>'+puterBalanceLabel(purchasedCredits,unit)+'</strong><span>Usati: '+puterCreditsLabel(purchasedUsed,unit)+' · Residui: '+puterCreditsLabel(purchasedRemaining,unit)+'</span>';
 ...
 const source=consumed.confidence==='derived'?'consumo ricavato dal saldo':'consumo comunicato da Puter';$('puterUsageDetail').textContent='La barra mostra il saldo globale residuo rispetto alla capacità nota (allowance + top-up). Puter scala prima l’allowance mensile, poi i crediti acquistati. '+source+' · aggiornato '+new Date().toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})+'. L’uso chicCanva è una lettura separata limitata a questa app.'
}
...
aiCreditText=function(value){return Math.round(aiEquivalentCredits(value)).toLocaleString(typeof appLocale==='function'?appLocale():'it-IT')+' '+(typeof appLanguage!=='undefined'&&appLanguage==='en'?'credits':'crediti')};
...
function aiEstimateMethodText(q){const english=typeof appLanguage!=='undefined'&&appLanguage==='en',kind=q.spec?.kind;if(kind==='openai-token-image')return english?'OpenAI output: official quality-grid and size formula.'+(q.ref?' Reference: visual-token proxy; the measured post-generation cost remains authoritative.':''):'Output OpenAI: formula ufficiale basata su griglia qualità e dimensioni.'+(q.ref?' Riferimento: proxy a token visivi; resta autorevole il consumo misurato dopo la generazione.':'');if(kind==='per-image-tier')return english?'xAI: fixed output price for the selected 1K/2K tier.'+(q.ref?' The official media-input fee is added.':''):'xAI: tariffa output fissa per il livello 1K/2K selezionato.'+(q.ref?' Si aggiunge la tariffa ufficiale per il riferimento.':'');if(kind==='per-image')return english?'Fixed Puter price per Seedream generation; no separate reference surcharge is published.':'Tariffa Puter fissa per generazione Seedream; non è pubblicato un sovrapprezzo separato per il riferimento.';if(kind==='gemini-token-image')return english?'Gemini: output image tokens at the official 1K rate.'+(q.ref?' Reference estimated with Google’s visual-tile token rules.':''):'Gemini: token immagine output alla tariffa ufficiale 1K.'+(q.ref?' Riferimento stimato con le regole Google per i tile visivi.':'');return english?'Fallback estimate from the embedded price list.':'Stima di riserva dal prezziario incorporato.'}
updateAiEstimate=function(){
 const box=$('aiCostEstimate');if(!box)return;const english=typeof appLanguage!=='undefined'&&appLanguage==='en',q=aiEstimateBreakdown(),dimensions=q.dimensions,parts=[(english?'output':'output')+' ~'+aiCreditText(q.outputUsd)];if(q.promptUsd)parts.push((english?'text':'testo')+' ~'+aiCreditText(q.promptUsd));if(q.referenceUsd)parts.push((english?'reference':'riferimento')+' ~'+aiCreditText(q.referenceUsd));let details=dimensions.valid===false?(english?'invalid output':'output non valido'):(english?'output':'output')+' ~'+dimensions.width+' × '+dimensions.height+' px · '+(english?'full prompt':'prompt complessivo')+' ~'+q.tokens+' token';if(q.ref){const w=Math.max(1,Math.round((q.ref.width||0)*q.factor)),h=Math.max(1,Math.round((q.ref.height||0)*q.factor));details+=' · '+(english?'reference':'riferimento')+' '+w+' × '+h+' px'}
 const listEstimate='~ '+aiCreditText(q.usd)+' · '+aiUsdText(q.usd),actual=$('aiLastGeneration'),last=puterActualCostCache()[aiEstimateSignature(q)];$('aiCostEstimateValue').textContent=last?(english?'Last generation: ':'Ultima Gen: ')+Math.round(last.credits).toLocaleString(typeof appLocale==='function'?appLocale():'it-IT')+' '+(english?'credits':'crediti')+' · '+aiUsdText(last.usd):(english?'Price-list estimate: ':'Stima Listino: ')+listEstimate;$('aiCostEstimateDetail').innerHTML=parts.join(' · ')+'<br>'+details+' · '+(english?'confidence ':'confidenza ')+q.confidence+'<br>'+aiEstimateMethodText(q);if(actual){actual.classList.toggle('hidden',!last);actual.textContent=last?(english?'Price-list estimate: ':'Stima Listino: ')+listEstimate:''}
};
