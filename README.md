...
Con Coolify dietro Traefik apri **Configuration → Domains → Search engine indexing**, scegli **Indexable** e ridistribuisci. La PWA blocca poi l’indicizzazione dal progetto: Googlebot e Bingbot possono leggere i rispettivi meta `noindex`, TelegramBot può leggere i metadati Open Graph e gli altri crawler ricevono `Disallow` dal `robots.txt`. Nessun `X-Robots-Tag` generico viene emesso dalla build, perché potrebbe impedire le anteprime social. Dopo il deploy verifica che l’header Coolify non sia presente e che `chiccanva-share.jpg` risponda pubblicamente.
...
- Sopra il pulsante di generazione compare una stima `~` in Credits interi e dollari a tre decimali, ricavata da modello, qualità, prompt, risoluzione e riferimento. Il listino riconosciuto viene conservato per 30 giorni nel browser e può essere aggiornato manualmente; errori di rete mantengono l’ultima copia valida o il listino incorporato. Il dato effettivo resta quello dell’account dopo la richiesta.
